    ...
    public function showSanphamList()
{
    ...
    public function showChitietSanphamlist()
    {
        $danhMucs = $this->model->getDanhMucs();
        $id = $_GET['id'] ?? null;
        ...
        if ($id && is_numeric($id)) {
            ...
            $Sanpham = $this->model->getSpById($id);
    
            // Lấy danh sách ảnh của sản phẩm
            $listSanpham = $this->model->getlistAnh($id);
    
            // Lấy bình luận của sản phẩm
            $binhLuans = $this->model->getBinhLuansBySanPhamId($id); 
    
            // Lấy đánh giá của sản phẩm
            $danhGias = $this->model->getDanhGiasBySanPhamId($id); // Hàm mới cần tạo trong model
